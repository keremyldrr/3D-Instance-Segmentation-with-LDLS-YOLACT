...
class Projection(object):
    ...
    def project_livox(self,points):
        ...
        cam_points = np.matmul(self.transformation_matrix,homo_points.T).T
        unnormalized = np.matmul(self.projection_matrix,cam_points.T)[:3,:]
        im_points = (unnormalized[:2,:]/unnormalized[2,:]).T
        #im_points[inds] = np.nan
        return im_points
